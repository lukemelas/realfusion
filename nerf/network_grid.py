...
class NeRFNetwork(NeRFRenderer):
    def __init__(self, 
        ...
        # masking
        self.grid_levels_mask = self.opt.grid_levels_mask
        ...
        if self.bg_radius > 0:
            ...
            # self.encoder_bg, self.in_dim_bg = get_encoder('tiledgrid', input_dim=2, num_levels=4, desired_resolution=desired_resolution)
            ...
            self.bg_net = MLP(self.in_dim_bg, self.C, hidden_dim_bg, num_layers_bg, bias=True)
    ...
    def common_forward(self, x):
        ...
        # sigma
        h: torch.Tensor = self.encoder(x, bound=self.bound)  # (N, self.in_dim)
    ...
    def forward(self, x, d, l=None, ratio=1, shading='albedo', albedo_only=False):
        # x: [N, 3], in [-bound, bound]
        ...
        if shading == 'albedo':
            ...
            normal = None if albedo_only else self.normal(x)
