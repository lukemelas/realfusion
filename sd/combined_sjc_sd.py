...
class StableDiffusion(nn.Module):
    ...
    def train_step(self, emb, pred_rgb, noise=None, guidance_scale=100, lambda_grad=1.0, mask_grad=None, steps=1):
        ...
        # Now we will integrate the gradient computation from the second code
        chosen_σs = torch.tensor(1.0, device=self.device, dtype=torch.float32).reshape(-1, 1, 1, 1)
        zs = latent_model_input+ chosen_σs * noise
        # print(zs.shape)
        # print(latents.shape)
        # print(noise.shape)
        Ds = self.unet(zs, t, encoder_hidden_states=emb).sample

        # Compute the gradient
        grad2 = (Ds - zs) / chosen_σs
        grad2 = grad2.mean(0, keepdim=True)
        
        # Combine the gradients from both codes
        grad_combined = grad + grad2

        # Manually backward, since we omitted an item in grad and cannot simply autodiff.
        latents.backward(gradient=grad_combined, retain_graph=True)

        # Return something to represent the loss
        pseudo_loss = grad_combined.abs().mean().detach()
        return pseudo_loss
